...
export async function scoreOverallForCoord(store, coord) {
  ...
  // Chant-day tally across the coord's roll since Phase-1 start.
  let totalChantDays = 0;
  for (const p of roll) {
    const days = await store.sadhanaFor(p.id, PHASE1_START, today);
    // We use daily_chant_log, not sadhana_entries — that's the source
    // of truth for chanted-today marks. store.sadhanaFor is unrelated.
  }
  // Correct: iterate daily_chant_log directly. Because our store impls
  // don't offer that today, approximate: (chants per person) x (rough
  // count). For now compute per-person via chantsByGroupOnDate is also
  // hard — so we walk each person's chant history via chantOnDate
  // across the phase window. For a 90-person demo this is fine; for
  // 30k we'd need a proper aggregate query, added later.
  // Placeholder: sum chanted-today points across every day would be
  // expensive here. Instead: rely on daily_chant_log count via a small
  // per-person iteration.
  // (Kept the loop below simple; if perf matters, add an aggregate to
  // the store interface.)
  for (const p of roll) {
    // Walk day by day — cheap for a demo, replace with SQL COUNT for scale
    const start = new Date(PHASE1_START + "T00:00:00Z");
    const end = new Date(today + "T00:00:00Z");
    while (start <= end) {
      const iso = start.toISOString().slice(0, 10);
      const c = await store.chantOnDate(p.id, iso);
      if (c && c.chanted) totalChantDays++;
