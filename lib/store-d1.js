...
export function d1Store(db) {
  ...
  return {
    ...
    async peopleAssignedTo(userId) {
      return all(
        `SELECT * FROM people
         WHERE assigned_to_user_id = ? AND active = 1 AND not_interested = 0
         ORDER BY legal_name`,
        userId,
      );
    },
    ...
    async searchPeople(query, limit = 50) {
      ...
      // sl_no can be alphanumeric ("30003a", "90-2760a") — match on the raw
      // trimmed query, lowercased, so operators can type it in any case.
      const slLike = `%${q.toLowerCase()}%`;
      return all(
        `SELECT * FROM people
         WHERE active = 1
           AND (LOWER(legal_name) LIKE ?
             OR (? != '' AND REPLACE(REPLACE(REPLACE(phone,'+',''),'-',''),' ','') LIKE ?)
             OR (sl_no IS NOT NULL AND LOWER(CAST(sl_no AS TEXT)) LIKE ?))
         ORDER BY legal_name
         LIMIT ?`,
        like, digLike, digLike, slLike, limit,
      );
    },
    async peopleCount() {
    ...
    // Conditional upgrade — used by the broadcast queue's Sent-tap.
    // Only moves contact_state from 0 (fresh) → 1 (contacted). Never
    // downgrades a person already at 2 (responded) or higher — the
    // coord clicking Sent in a broadcast is weaker evidence than the
    // responded/needs_visit state a coord set by hand earlier.
    async markContactedIfLower(personId, byUserId) {
      const existing = await this.personById(personId);
      if (!existing) return { ok: false, reason: "not_found" };
      const now = nowIso();
      // Guard: only upgrade when currently 0 (fresh/uncontacted).
      const r = await run(
        `UPDATE people
            SET contact_state = 1,
                last_marked_at = ?,
                last_marked_by = ?,
                updated_at     = ?
          WHERE id = ? AND active = 1 AND contact_state = 0`,
        now, byUserId, now, personId,
      );
      const changed = !!(r && r.meta && r.meta.changes);
      const after = changed ? await this.personById(personId) : existing;
      return {
        ok: true,
        changed,
        contact_state: after.contact_state || 0,
        prior_contact_state: existing.contact_state || 0,
      };
    },
    ...
    // ---------- fast HK-leaders rollup ----------
    // ONE query returns (coord_id → count) for people in every coord's roll.
    async peopleCountByCoord() {
      const rows = await all(
        `SELECT assigned_to_user_id AS coord_id, COUNT(*) AS n
         FROM people WHERE active = 1 AND assigned_to_user_id IS NOT NULL
         GROUP BY assigned_to_user_id`,
      );
      const map = new Map();
      for (const r of rows) map.set(r.coord_id, r.n);
      return map;
    },
    // ONE query returns (coord_id → count of persons whose last_marked_at
    // is on `dateIso`). Approximates today's follow-ups per coord.
    async followUpsCountByCoordOnDate(dateIso) {
      const rows = await all(
        `SELECT assigned_to_user_id AS coord_id, COUNT(*) AS n
         FROM people
         WHERE active = 1 AND assigned_to_user_id IS NOT NULL
           AND substr(last_marked_at, 1, 10) = ?
         GROUP BY assigned_to_user_id`,
        dateIso,
      );
      const map = new Map();
    ...
    // ONE query returns (coord_id → count of DAILY-STATUS persons who
    // chanted on `dateIso`). Used for the primary "SKJ daily-committed
    // chanters who chanted today" scorecard.
    async chantedTodayDailyByCoord(dateIso) {
      const rows = await all(
        `SELECT p.assigned_to_user_id AS coord_id, COUNT(DISTINCT p.id) AS n
         FROM daily_chant_log d
         JOIN people p ON p.id = d.person_id
         WHERE p.active = 1 AND p.status = 'daily'
           AND p.assigned_to_user_id IS NOT NULL
           AND d.entry_date = ? AND d.chanted = 1
    ...
    // ONE query returns (coord_id → count of daily-status persons who
    // have chanted at least once in the last `minWindow` days ending
    // today). "3+ consecutive misses → disqualified" rule: default
    // window is 3, so anyone with 0 chants in the last 3 days doesn't
    // count as consistent.
    async consistentDailyByCoord(todayIso, missWindowDays = 3) {
      const start = new Date(todayIso + "T00:00:00Z");
      start.setUTCDate(start.getUTCDate() - (missWindowDays - 1));
      const startIso = start.toISOString().slice(0, 10);
      const rows = await all(
        `SELECT p.assigned_to_user_id AS coord_id, COUNT(DISTINCT p.id) AS n
         FROM people p
         WHERE p.active = 1 AND p.status = 'daily'
           AND p.assigned_to_user_id IS NOT NULL
           AND EXISTS (
             SELECT 1 FROM daily_chant_log d
             WHERE d.person_id = p.id AND d.chanted = 1
               AND d.entry_date >= ? AND d.entry_date <= ?
           )
         GROUP BY p.assigned_to_user_id`,
        startIso, todayIso,
    ...
    // ONE query returns (coord_id → distinct-persons-chanted) for `dateIso`.
    async chantedCountByCoordOnDate(dateIso) {
      const rows = await all(
        `SELECT p.assigned_to_user_id AS coord_id, COUNT(DISTINCT p.id) AS n
         FROM daily_chant_log d
         JOIN people p ON p.id = d.person_id
    ...
    // ONE query returns (coord_id → count of daily-status chanters).
    async dailyChanterCountByCoord() {
      const rows = await all(
        `SELECT assigned_to_user_id AS coord_id, COUNT(*) AS n
         FROM people WHERE active = 1 AND status = 'daily'
    ...
    // ONE query returns (coord_id → count of persons with ≥ minDays chants
    // in the window). Approximates the per-coord one-month-daily metric.
    async oneMonthDailyCountByCoord(todayIso, windowDays = 30, minDays = 25) {
      const start = new Date(todayIso + "T00:00:00Z");
      start.setUTCDate(start.getUTCDate() - windowDays + 1);
      const startIso = start.toISOString().slice(0, 10);
      const rows = await all(
        `SELECT p.assigned_to_user_id AS coord_id, COUNT(*) AS n FROM (
           SELECT p2.id AS pid, p2.assigned_to_user_id AS coord_id, COUNT(*) AS days
           FROM daily_chant_log d
           JOIN people p2 ON p2.id = d.person_id
           WHERE p2.active = 1 AND p2.status = 'daily'
             AND p2.assigned_to_user_id IS NOT NULL
             AND d.chanted = 1
             AND d.entry_date >= ? AND d.entry_date <= ?
           GROUP BY p2.id
           HAVING COUNT(*) >= ?
         ) sub
         JOIN people p ON p.id = sub.pid
         GROUP BY p.assigned_to_user_id`,
        startIso, todayIso, minDays,
      );
      const map = new Map();
      for (const r of rows) map.set(r.coord_id, r.n);
      return map;
    ...
    // Bulk-fetch chanted=1 rows for a set of persons in a date range.
    // Returns [{person_id, entry_date}] — callers bucket client-side.
    // Used by care-moments to avoid N*M chantOnDate() subrequests.
    async chantDatesForPersonIdsBetween(personIds, fromIso, toIso) {
      if (!personIds || personIds.length === 0) return [];
      const placeholders = personIds.map(() => "?").join(",");
      return all(
        `SELECT person_id, entry_date FROM daily_chant_log
         WHERE chanted = 1
           AND entry_date >= ? AND entry_date <= ?
           AND person_id IN (${placeholders})`,
        fromIso, toIso, ...personIds,
      );
    },
    ...
    // ---------------- Members tab — single-shot table load ----------------
    // Return every active person with the columns the Members-tab table
    // needs, plus a LEFT JOIN on users so the assigned coordinator's
    // display_name comes back in the same row (no N+1 lookups). Rows are
    // ordered by name so the frontend can render without a resort.
    async listAllPeopleWithAssignments() {
      return all(
        `SELECT p.id, p.legal_name, p.phone, p.pincode, p.sl_no, p.gender,
                p.assigned_to_user_id, p.not_interested,
                u.display_name AS assigned_coord_name,
                u.manager_user_id AS assigned_coord_manager_id
         FROM people p
         LEFT JOIN users u ON u.id = p.assigned_to_user_id AND u.active = 1
         WHERE p.active = 1
         ORDER BY p.legal_name`,
      );
    },
    ...
    async sadhanaById(id) {
      return one(`SELECT * FROM sadhana_entries WHERE id = ?`, id);
    },
    ...
    async removeWebPushSub(endpoint, userId) {
      if (userId) {
        await run(
          `UPDATE web_push_subscriptions SET active = 0 WHERE endpoint = ? AND user_id = ?`,
          endpoint, userId,
        );
      } else {
    ...
    // ---------------- leader touch actions ----------------
    // See schema/0010_leader_touch.sql for the ledger shape. Points and
    // uniqueness rules live in application code (lib/handlers.js) —
    // the store just enforces the composite unique index and returns
    // aggregates for the 3-bucket prorated leader leaderboard.
    async logLeaderTouch({ leaderUserId, actionKey, points, dayIso, refId }) {
      const now = nowIso();
      const row = {
        id: newId(),
      ...
      // OR IGNORE swallows the UNIQUE(leader,action,day,ref_id) collision
      // so callers can fire this on every eligible event without a
      // pre-check round-trip. Returns whether a row was actually inserted.
      const r = await run(
        `INSERT OR IGNORE INTO leader_touch_actions
           (id, leader_user_id, action_key, awarded_at, day_iso, points, ref_id)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        row.id, row.leader_user_id, row.action_key,
        row.awarded_at, row.day_iso, row.points, row.ref_id,
      );
      const inserted = !!(r && r.meta && r.meta.changes);
      return { inserted, row };
    },
    // Count of rows this leader already has today for a given action_key
    // — used by the +3-per-coord-per-day cap on contact_coord logging.
    async countLeaderTouchForDay(leaderUserId, actionKey, dayIso) {
      const r = await one(
        `SELECT COUNT(*) AS n FROM leader_touch_actions
          WHERE leader_user_id = ? AND action_key = ? AND day_iso = ?`,
        leaderUserId, actionKey, dayIso,
      );
      return (r && r.n) || 0;
    },
    // All of a leader's touch rows for one day. Callers roll up to
    // buckets (per action_key) for the /api/leader/points-breakdown
    // response so the UI can show the per-action tally.
    async leaderTouchRowsForDay(leaderUserId, dayIso) {
      return all(
        `SELECT action_key, points, ref_id, awarded_at
           FROM leader_touch_actions
          WHERE leader_user_id = ? AND day_iso = ?
          ORDER BY awarded_at`,
        leaderUserId, dayIso,
      );
    },
    // ONE query returns (leader_user_id → total touch points that day).
    // Feeds the leader leaderboard's 15%-weight Leader-Touch bucket in
    // a single round-trip rather than N per-leader queries.
    async leaderTouchSumByLeaderForDay(dayIso) {
      const rows = await all(
        `SELECT leader_user_id AS leader_id, SUM(points) AS pts,
                COUNT(*) AS n
           FROM leader_touch_actions
          WHERE day_iso = ?
      ...
      const map = new Map();
      for (const r of rows) map.set(r.leader_id, { pts: r.pts || 0, n: r.n || 0 });
      return map;
    },
    // Cumulative Leader-Touch across all days (overall board).
    async leaderTouchSumByLeaderAll() {
      const rows = await all(
        `SELECT leader_user_id AS leader_id, SUM(points) AS pts, COUNT(*) AS n
           FROM leader_touch_actions
          GROUP BY leader_user_id`,
      );
      const map = new Map();
    ...
    // ---------------- duplicate requests ----------------
    // Coord flags one member as a duplicate of another (by SL). See
    // schema/0019_duplicate_requests.sql for the row shape.
    async createDuplicateRequest(fields) {
      const now = nowIso();
      const row = {
        id: fields.id || newId(),
        status: "pending",
      ...
      await run(
        `INSERT INTO duplicate_requests
           (id, flagged_person_id, duplicate_of_person_id, duplicate_of_sl,
            note, status, requested_by, reviewed_by, resolution_note,
            merged_kept_id, merged_deactivated_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        row.id, row.flagged_person_id, row.duplicate_of_person_id ?? null,
        row.duplicate_of_sl ?? null, row.note ?? null, row.status,
        row.requested_by, row.reviewed_by ?? null, row.resolution_note ?? null,
        row.merged_kept_id ?? null, row.merged_deactivated_id ?? null,
        row.created_at, row.updated_at,
      );
      return row;
    },
    async duplicateRequestById(id) {
      return one(`SELECT * FROM duplicate_requests WHERE id = ?`, id);
    },
    // Look for an OPEN (pending) request on this flagged_person_id.
    // Used both to gate duplicate submissions (don't let a coord flag
    // the same row twice while the first is still pending) and to power
    // the "🔍 Duplicate flagged" pill on the coord's My Sangha rows.
    async pendingDuplicateRequestForPerson(personId) {
      return one(
        `SELECT * FROM duplicate_requests
          WHERE flagged_person_id = ? AND status = 'pending'
          ORDER BY created_at DESC LIMIT 1`,
        personId,
      );
    },
    // ONE query returns the set of flagged_person_ids that currently have
    // a pending duplicate_request. Handy for the roll renderer to decorate
    // rows without an N+1 lookup.
    async pendingDuplicatePersonIdsForCoord(coordId) {
      const rows = await all(
        `SELECT DISTINCT d.flagged_person_id
           FROM duplicate_requests d
           JOIN people p ON p.id = d.flagged_person_id
          WHERE d.status = 'pending' AND p.assigned_to_user_id = ?`,
        coordId,
      );
      return rows.map(r => r.flagged_person_id);
    },
    // Count pending requests visible to a leader. "Visible" == the flagged
    // person is owned by one of THIS leader's coords. Powers the info
    // line on the Team page. Zero when the leader has no coords.
    async pendingDuplicateCountForLeader(leaderId) {
      const r = await one(
        `SELECT COUNT(*) AS n
           FROM duplicate_requests d
           JOIN people p ON p.id = d.flagged_person_id
           JOIN users  u ON u.id = p.assigned_to_user_id
          WHERE d.status = 'pending' AND u.manager_user_id = ?`,
        leaderId,
      );
      return (r && r.n) || 0;
    },
    // Count of pending requests across the whole install — powers the
    // HK nav badge.
    async pendingDuplicateCountAll() {
      const r = await one(
        `SELECT COUNT(*) AS n FROM duplicate_requests WHERE status = 'pending'`,
      );
      return (r && r.n) || 0;
    },
    // List duplicate requests with joined display info so the queue can
    // render side-by-side (flagged vs. duplicate_of) without N+1 hits.
    // Filter by status; optional restriction to a set of flagged person
    // ids (leader / coord scope) or a specific requester.
    async listDuplicateRequests({ status, flaggedPersonIds, requestedBy } = {}) {
      const wh = [];
      const binds = [];
      if (status) { wh.push(`d.status = ?`); binds.push(status); }
      if (requestedBy) { wh.push(`d.requested_by = ?`); binds.push(requestedBy); }
      if (Array.isArray(flaggedPersonIds)) {
        if (flaggedPersonIds.length === 0) return [];
        const ph = flaggedPersonIds.map(() => "?").join(",");
        wh.push(`d.flagged_person_id IN (${ph})`);
        binds.push(...flaggedPersonIds);
      }
      const where = wh.length ? `WHERE ${wh.join(" AND ")}` : "";
      return all(
        `SELECT d.*,
                pf.legal_name AS flagged_name, pf.sl_no AS flagged_sl,
                pf.phone      AS flagged_phone,
                pd.legal_name AS dup_name,     pd.sl_no AS dup_sl,
                pd.phone      AS dup_phone,
                rb.display_name AS requested_by_name
           FROM duplicate_requests d
           LEFT JOIN people pf ON pf.id = d.flagged_person_id
           LEFT JOIN people pd ON pd.id = d.duplicate_of_person_id
    ...
    // Resolve one request — merge or reject. Merge deactivates the loser
    // in the SAME logical operation so the queue and the people table
    // stay consistent. D1 has no client-side transactions here; we do
    // the deactivation before the status update, so a mid-flight failure
    // leaves the request as pending (safe re-try) rather than a "merged"
    // request whose loser is still active.
    async resolveDuplicateRequest(id, { action, reviewedBy, keepPersonId, resolutionNote }) {
      const req = await this.duplicateRequestById(id);
      if (!req) return { ok: false, reason: "not_found" };
      if (req.status !== "pending") return { ok: false, reason: "already_resolved", status: req.status };
      const now = nowIso();
      if (action === "reject") {
        await run(
          `UPDATE duplicate_requests
              SET status = 'rejected',
                  reviewed_by = ?, resolution_note = ?, updated_at = ?
            WHERE id = ?`,
          reviewedBy || null, resolutionNote || null, now, id,
        );
        return { ok: true, status: "rejected" };
      }
      if (action !== "merge") return { ok: false, reason: "bad_action" };
      const flagged = req.flagged_person_id;
      const dupOf   = req.duplicate_of_person_id;
      if (!flagged || !dupOf) return { ok: false, reason: "merge_needs_both_ids" };
      if (keepPersonId !== flagged && keepPersonId !== dupOf) {
        return { ok: false, reason: "keep_must_be_one_of_the_two" };
      }
      const loserId = keepPersonId === flagged ? dupOf : flagged;
      // Deactivate the loser (soft-delete). History (chants, attendance,
      // sadhana) is preserved by design — the row simply drops off active
      // listings.
      await run(
      ...
      await run(
        `UPDATE duplicate_requests
            SET status = 'merged',
                ...
                merged_kept_id = ?, merged_deactivated_id = ?,
                updated_at = ?
          WHERE id = ?`,
        reviewedBy || null, resolutionNote || null,
        keepPersonId, loserId, now, id,
      );
      return { ok: true, status: "merged", kept: keepPersonId, deactivated: loserId };
    },
    // Lookup a person by their sl_no (canonical string form). Used by
    // the /api/duplicate-requests POST to resolve the coord-typed SL
    // into a person_id before writing the row. Match is case-insensitive
    // on the trimmed text, so operators typing "2760" find "2760" or
    // "2760a" etc. Returns the first active match.
    async personBySlNo(slNo) {
      const q = String(slNo || "").trim();
      if (!q) return null;
      return one(
        `SELECT * FROM people
          WHERE active = 1
            AND sl_no IS NOT NULL
            AND LOWER(CAST(sl_no AS TEXT)) = LOWER(?)
          LIMIT 1`,
        q,
      );
    },
