...
// coord users, finds the highest sl_range_end below 40000, adds 100.
// If no coord has a range yet, seeds at 10000.
reg("GET", "/api/admin/next-sl-range", async (req, ctx) => {
  await requireFeature(ctx.store, ctx.user.role, "feature_admin");
  const users = await ctx.store.listAllUsers();
  const ends = users
    .map(u => Number(u.sl_range_end))
    .filter(n => Number.isFinite(n) && n < 40000);
  const nextStart = ends.length ? Math.max(...ends) + 1 : 10000;
...
async function assignAndCreate(store, coord, { name, mobile, pincode }) {
  ...
  // Same partial-unique-index dedup rule the bulk-import path uses.
  const dup = await store.personByPhone(mobile);
  if (dup) return { error: "duplicate_phone", phone: mobile };
