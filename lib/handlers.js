...
reg("GET", "/api/roll/care-moments", async (req, ctx) => {
  ...
  // Bulk-fetch chant logs for all daily people in one pass — 30-day window
  const windowStart = daysAgoIso(today, 29);
  for (const p of dailyPeople) {
    // Streak: count consecutive days back from today with chanted=1
    let streak = 0;
    for (let i = 0; i < 60; i++) {
      const day = daysAgoIso(today, i);
      const c = await ctx.store.chantOnDate(p.id, day).catch(() => null);
      if (c && c.chanted) { streak += 1; }
      else break;
    }
    // Days since last chant
    let daysSince = null;
    const chantedInWindow = await ctx.store.hasChantedSince(p.id, windowStart);
    if (!chantedInWindow) {
      daysSince = 30; // capped, "not in last month"
    } else {
      // Walk back from today to find the most-recent chant
      for (let i = 0; i < 30; i++) {
        const day = daysAgoIso(today, i);
        const c = await ctx.store.chantOnDate(p.id, day).catch(() => null);
        if (c && c.chanted) { daysSince = i; break; }
      }
    }
