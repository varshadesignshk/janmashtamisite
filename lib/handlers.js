...
// Leader Touch: leader confirms they messaged a specific coord via
// mesh WhatsApp (client-side link — server can't observe the send).
// +3 per coord per day, capped at 3 coords per leader per day.
// Only applies to njy_leader — HK doesn't compete on this board.
reg("POST", "/api/leader/contact-log", async (req, ctx) => {
  if (ctx.user.role !== "njy_leader") return bad(403, "forbidden");
  const body = await req.json().catch(() => ({}));
  const { coord_user_id } = body;
  if (!coord_user_id) return bad(400, "coord_user_id_required");
  const coord = await ctx.store.userById(coord_user_id);
  if (!coord) return bad(404, "coord_not_found");
  // Guard: leader can only claim points for their OWN coords.
  if (coord.manager_user_id !== ctx.user.id) return bad(403, "not_your_coord");
  const today = todayInTz(ctx.env?.APP_TZ);
  const r = await awardLeaderTouch(ctx.store, ctx.user, "contact_coord", today, coord_user_id);
