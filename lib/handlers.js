...
reg("GET", "/api/members/list", async (req, ctx) => {
  ...
  // Coords the caller may assign people TO.
  let eligible_coords = [];
  if (role === "hk_leader") {
    eligible_coords = allCoords.filter(c => c.active).map(c => ({
      id: c.id, display_name: c.display_name || c.username,
    }));
  } else if (role === "njy_leader") {
    eligible_coords = allCoords
      .filter(c => c.active && c.manager_user_id === meId)
      .map(c => ({ id: c.id, display_name: c.display_name || c.username }));
  }
...
reg("GET", "/api/members/eligible-coords", async (req, ctx) => {
  ...
  return json({ coords: eligible.map(c => ({ id: c.id, display_name: c.display_name || c.username })) });
}, { auth: true });
