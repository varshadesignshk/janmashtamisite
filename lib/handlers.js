...
reg("GET", "/api/leader/coordinators", async (req, ctx) => {
  ...
  // NJY Leader sees only their own coords (filtered by manager_user_id).
  // HK Leader sees everyone.
...
reg("GET", "/api/hk/leaders", async (req, ctx) => {
  ...
  const rows = await Promise.all(leaders.map(async (l) => {
    ...
    for (const c of myCoords) {
      ...
      assigned += roll.length;
      let coordChanted = 0;
      for (const p of roll.slice(0, 500)) {
        const chant = await ctx.store.chantOnDate(p.id, today).catch(() => null);
        if (chant && chant.chanted) { coordChanted += 1; }
      }
      chantedToday += coordChanted;
      if (coordChanted > 0) activeCoords += 1;
      const m = await ctx.store.oneMonthDailyCountFor(c.id, today).catch(() => ({ one_month_daily: 0, daily_chanter_total: 0 }));
      oneMonthDaily += m.one_month_daily || 0;
      dailyTotal += m.daily_chanter_total || 0;
    }
    return {
...
reg("GET", "/api/leaderboard/leaders/daily", async (req, ctx) => {
  ...
  const today = todayInTz(ctx.env?.APP_TZ);
  const rows = await leaderScope(ctx.store, today, "daily");
  return json({ date: today, rows });
}, { auth: true });
